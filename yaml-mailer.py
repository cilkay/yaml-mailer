...
parser.add_argument('--config', type=str,
                    default=path.expanduser("~")+"/.mailer.yaml")
...
check_key("Yaml config", yaml_config, 'paths')
paths = yaml_config['paths']

for key in ['messages_path', 'attachments_path']:
    check_key("Yaml config - paths", paths, key)

# Message file
message_file = path.join(paths['messages_path'], args.messages_yaml)
...
for msg in messages:
    ...
    if 'cc' in msg:
        multi_msg['CC'] = ",".join(msg['cc'])
    multi_msg.attach(MIMEText(msg['contents']))

    if 'attach' in msg:
        for f_name in msg['attach']:
            part = MIMEBase('application', "octet-stream")
            attachment_file = path.join(paths['attachments_path'], f_name)
            with open(attachment_file, 'rb') as f:
                part.set_payload(f.read())
            encode_base64(part)
            part.add_header('Content-Disposition',
                            'attachment; filename="' + path.basename(f_name) + '"')
    ...
    if 'cc' in msg:
        print("    CC: " + ",".join(msg['cc']))

    if 'bcc' in msg:
        print("    BCC: " + ",".join(msg['bcc']))

    print("    Subject: " + msg['subject'])

    if 'attach' in msg:
        print("    Attach: " + ",".join(msg['attach']))
